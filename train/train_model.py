...
parser.add_argument('--pre', default='resnet', type=str, help='pre processing for image input')
parser.add_argument('--oversample', default=True, type=bool, help='whether to oversample')
parser.add_argument('--root_dir', default='/rds/general/user/emuller/home/emily/phd/', help='path to recode-perceptions')
parser.add_argument('--lr', default=1e-3,type=float, help='learning rate')
parser.add_argument('--study_id', default='50a68a51fdc9f05596000002', type = str, help='perceptions_1_to_6')
parser.add_argument('--run_name', default='default',type=str, help='unique name to identify hyperparameter choices')
...
def main():
    ...
    validation_gen = CustomImageDataset(df_val, root_dir=opt.root_dir + opt.data_dir, transform=opt.pre)
    test_gen = CustomImageDataset(df_test, root_dir=opt.root_dir + opt.data_dir, transform=opt.pre)
    ...
    # Train model
    train_val_loss = train.train(model=model, 
    ...
    test_loss = train.test_step(model=model, 
                        test_dataloader=validation_dataloader,
                        loss_fn=loss_fn, 
                        device=device)
    print(f"Model tested in: {timer()-end_time:.3f} seconds")
    
    print('LOSS train {} valid {} test {}'.format(train_val_loss["train_loss"][-1], train_val_loss["val_loss"][-1], test_loss))
