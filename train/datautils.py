...
def add_qscore(root_dir, images_df, perception_study, metadata = 'input/meta/qscores.tsv'):
    ...
    images_df.insert(0, 'trueskill.score', images_df['location_id'].map(perception_meta.set_index('location_id')['trueskill.score'])) 
    return images_df
...
def scale_data(images_df, start, end):
    ...
    images_df['trueskill.score_norm'] = images_df['trueskill.score'].apply(lambda x: ( x - min_) / (max_ - min_) * width + start)
    return images_df
